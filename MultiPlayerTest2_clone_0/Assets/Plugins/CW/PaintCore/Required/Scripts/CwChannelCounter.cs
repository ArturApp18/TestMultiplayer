...
namespace PaintCore
{
	...
	public class CwChannelCounter : CwPaintableTextureMonitorMask
	{
		...
		[Range(0.0f, 1.0f)] [SerializeField] private float threshold = 0.5f;
		...
		[SerializeField] private int countR;
		...
		[SerializeField] private int countG;
		...
		[SerializeField] private int countB;

		/// <summary>The previously counted amount of pixels with a alpha channel value above the threshold.</summary>
		public event Action<int> OnCountAChanged;

		[SerializeField] private int countA;
		...
		public float RatioR
		{
			get { return total > 0 ? countR / (float)total : 0.0f; }
		}
		...
		public float RatioG
		{
			get { return total > 0 ? countG / (float)total : 0.0f; }
		}
		...
		public float RatioB
		{
			get { return total > 0 ? countB / (float)total : 0.0f; }
		}
		...
		public float RatioA
		{
			get { return total > 0 ? countA / (float)total : 0.0f; }
		}
		...
		public static long GetCountR(ICollection<CwChannelCounter> counters = null)
		{
			...
			foreach (var counter in counters ?? Instances)
			{
				if (counter != null) solid += counter.countR;
			}
		...
		public static long GetCountG(ICollection<CwChannelCounter> counters = null)
		{
			...
			foreach (var counter in counters ?? Instances)
			{
				if (counter != null) solid += counter.countG;
			}
		...
		public static long GetCountB(ICollection<CwChannelCounter> counters = null)
		{
			...
			foreach (var counter in counters ?? Instances)
			{
				if (counter != null) solid += counter.countB;
			}
		...
		public static long GetCountA(ICollection<CwChannelCounter> counters = null)
		{
			...
			foreach (var counter in counters ?? Instances)
			{
				if (counter != null) solid += counter.countA;
			}
		...
		protected override void HandleComplete(int boost)
		{
			...
			countG = 0;
			countB = 0;
			countA = 0; // Используем свойство вместо прямого присваивания
			total = 0;
			...
			for (var i = 0; i < currentPixels.Length; i++)
			{
				if (maskPixels[i] > 127)
				{
					...
					if (currentPixel32.a >= threshold32) countA++; // Оставляем как есть для инкремента
				}
			}
			...
			countG *= boost;
			countB *= boost;
			CountA = countA * boost; // Используем свойство для финального значения
			total *= boost;
